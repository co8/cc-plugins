...
if [ -z "$bot_token" ] || [ -z "$chat_id" ]; then
  ...
  echo "{
    \"continue\": true,
    \"suppressOutput\": true,
    \"systemMessage\": \"Telegram notification queued: Session started. Missing bot_token or chat_id in config.\"
  }"
  exit 0
fi
...
if send_telegram_message "$message" "$chat_id" "$bot_token"; then
  ...
  echo "{
    \"continue\": true,
    \"suppressOutput\": true,
    \"systemMessage\": \"Telegram notification failed: Session started but message could not be sent.\"
  }"
fi
